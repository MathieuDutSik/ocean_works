...
def DATE2JD(Date):
    ...
    eFracDay = (sec + 60 * minD + 3600 * (hour - 12)) / 86400.0
...
def JD2DATE(eJD):
    ...
    a = ijd + 32044
    b = math.floor((4.0 * a + 3.0) / 146097.0)
    c = a - math.floor((b * 146097.0) / 4.0)
    d = math.floor((4.0 * c + 3.0) / 1461.0)
    e = c - math.floor((1461.0 * d) / 4.0)
    m = math.floor((5.0 * e + 2.0) / 153.0)
    day = e - math.floor((153.0 * m + 2.0) / 5.0) + 1
    month = m + 3 - 12 * math.floor(m / 10.0)
    year = b * 100 + d - 4800 + math.floor(m / 10.0)
    #
    fjd = eJD - ijd + 0.5
    second = 86400.0 * fjd
    hour = math.floor(second / 3600.0)
    second = second - 3600.0 * hour
    minD = math.floor(second / 60.0)
    sec = math.floor(second - 60.0 * minD)
    #
    #    secNear = math.round(second - 60.*minD)
    secNear = int(round(second - 60.0 * minD))
    if secNear == 0:
        sec = 0
        minD = minD + 1
    if minD == 60:
