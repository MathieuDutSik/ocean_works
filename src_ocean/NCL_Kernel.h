...
double AveragePairLength(SeqLineSegment const& eSeq)
{
  ...
  for (int i=0; i<nb; i++) {
    int j=(i+1) % nb;
    double distKM = GeodesicDistanceKM_pair(eSeq.ListPairLL[i], eSeq.ListPairLL[j]);
    eSum += distKM;
  }
...
void ApplyPdfcrop(std::string const& PdfFile)
{
  ...
  std::string eCommand1="(cd " + TheDir + " && pdfcrop " + FileNaked + " " + TmpFile + ")";
...
void CALL_PICTURE_PROG(TripleNCL const& eTripl)
{
  if (eTripl.eBChoice.OnlyCreateFiles) {
    if (eTripl.eBChoice.PrintDebugInfo) {
      ...
      std::cerr << "   Only creating NC, PY, NCL files, so no operations needed\n";
    }
    return;
  }
  //
  ...
  if (eTripl.eBChoice.PrintDebugInfo)
    std::cerr << "eTripl.TargetFile=" << eTripl.TargetFile << " FinalTargetFile=" << FinalTargetFile << "\n";
  auto GetCommand=[&]() -> std::string {
	...
	return "(cd " + TheDir + " && " + eComm + " " + FILE_GetNakedFilename(eTripl.eFileNCL) + devNull + ")";
      }
      ...
      if (eTripl.eBChoice.InPlaceRun) {
	std::string TheDir = FILE_GetDirectoryOfFileName(eTripl.TargetFile);
	return "(cd " + TheDir + " && " + eComm + " " + FILE_GetNakedFilename(eTripl.eFileNCL) + devNull + " && mv " + eTripl.TargetFile + " " + FinalTargetFile + ")";
      }
      return "(" + eComm + " " + eTripl.eFileNCL + devNull + " && mv " + eTripl.TargetFile + " " + FinalTargetFile + ")";
    }
  };
...
struct NCLcaller {
  ...
  NCLcaller(int const& nproc) : NPROC(nproc)
  {
    ...
    for (int iProc=0; iProc<NPROC; iProc++)
      ListThr.push_back(std::thread(IterationLoop, iProc, &(ListExch[iProc]), &(ListTerm[iProc])));
    for (int iProc=0; iProc<NPROC; iProc++)
