...
void GRIB_CheckAllowedKeywords(std::string const& eModelName)
{
  ...
  std::vector<std::string> ListAllowed{"optimaltime", "shifttime", "timestartfromfilename", "retrieveallstates"};
...
void PrintGRIBmessageInfo(std::ostream & os, GRIB_MessageInfo const& eMesg)
{
  ...
  os << "  shortName=" << eMesg.shortName << "  |shortName|=" << eMesg.shortName.size() << "\n";
  os << "  cfVarName=" << eMesg.cfVarName << "\n";
  os << "  cfVarNameECMWF=" << eMesg.cfVarNameECMWF << "\n";
  os << "  name=" << eMesg.name << "\n";
  os << "  units=" << eMesg.units << "\n";
  os << "  time=" << eMesg.time << " string=" << DATE_ConvertMjd2mystringPres(eMesg.time) << "\n";
  os << "  timeStart=" << eMesg.timeStart << " string=" << DATE_ConvertMjd2mystringPres(eMesg.timeStart) << "\n";
  os << "  stepRange=" << eMesg.stepRange << "\n";
}


void PrintVectorGRIBmessageInfo(std::ostream & os, std::vector<GRIB_MessageInfo> const& ListMesg)
{
...
double ExtractTimeStartFromName(double const& PreTimeStart, double const& eTime, std::string const& eModelName, std::string const& FileName)
{
  std::vector<std::string> LStr=STRING_Split(eModelName, ":");
  for (int i=1; i<int(LStr.size()); i++) {
    if (LStr[i] == "timestartfromfilename") {
      ...
      if (LStrB.size() != 2) {
	std::cerr << "We cannot extract timestart from filename. Heuristic for ALADIN does not apply\n";
	throw TerminalException{1};
      }
	...
	WeMatch=true;
      ...
      if (!WeMatch) {
	std::cerr << "str=" << str << " but it should have length 6 or 8, i.e. be of the form 0048.grb or 48.grb\n";
	throw TerminalException{1};
      }
      double eValHour;
...
std::vector<GRIB_MessageInfo> GRIB_GetAllMessagesFromFile(std::string const& FileName, std::string const& eModelName)
{
  grib_handle *h = NULL;
  int err;
  FILE* in = NULL;
  ...
  while ((h = grib_handle_new_from_file(0,in,&err)) != NULL ) { 
    ...
    double eTimeStart=ExtractTimeStartFromName(PreTimeStart, eTime, eModelName, FileName);
    std::string strDate=DATE_ConvertMjd2mystringPres(eTime);
    //    std::cerr << "FileName=" << FileName << " year=" << year << " month=" << month << " day=" << day << " hour=" << hour << " min=" << min << " strDate=" << strDate << "\n";
    //
    //    std::cerr << "Inserting message FileName=" << FileName << "\n";
    //    std::cerr << "  idx=" << idx << "\n";
    //    std::cerr << "  ShortNameValue=" << ShortNameValue << "\n";
    //    std::cerr << "  eTime=" << eTime << "\n";
    GRIB_MessageInfo eInfo{ShortNameValue, cfVarName, cfVarNameECMWF, NameValue, units, idx, eTime, eTimeStart, stepRange, FileName};
...
bool GRIB_HasVariable(std::vector<GRIB_MessageInfo> const& ListMessage, std::string const& nameSearch, std::string const& VarName)
{
...
MyMatrix<double> GRIB_Read2Dvariable(std::vector<GRIB_MessageInfo> const& ListMessage, std::string const& nameSearch, std::string const& VarName)
{
  /*
  std::cerr << "|ListMessage|=" << ListMessage.size() << "\n";
  for (auto & eMesg : ListMessage) {
    std::cerr << "  eMesg : shortName = " << eMesg.shortName << "  eMesg.FileName = " << eMesg.FileName << "\n";
    }*/
  for (auto & eMesg : ListMessage) {
    if (nameSearch == "shortName" && eMesg.shortName == VarName)
...
MyMatrix<double> GRID_Get2DVariableTimeDifferentiate(TotalArrGetData const& TotalArr, std::string const& VarName, double const& eTimeDay)
{
  ...
  for (int iTimeStart=0; iTimeStart<nbTimeStart; iTimeStart++) {
	...
	ListShootSolution.push_back({iMesgLow, iMesgUpp, eTimeLow, eTimeUpp, eStartTime, DeltaTimeDay});
      }
    }
  }
...
MyMatrix<double> GRIB_Get2DvariableSpecTime(TotalArrGetData const& TotalArr, std::string const& nameSearch, std::string const& VarName, double const& eTimeDay)
{
  ...
  if (eInterpInfo.UseSingleEntry) {
    ...
    return GRIB_Read2Dvariable(TotalArr.eArr.ListListMessages[iTimeReal], nameSearch, VarName);
  }
  ...
  MyMatrix<double> eVarLow=GRIB_Read2Dvariable(TotalArr.eArr.ListListMessages[iTimeRealLow], nameSearch, VarName);
  //  std::cerr << "GRIB_Get2DvariableSpecTime, step 4\n";
  MyMatrix<double> eVarUpp=GRIB_Read2Dvariable(TotalArr.eArr.ListListMessages[iTimeRealUpp], nameSearch, VarName);
  //  std::cerr << "GRIB_Get2DvariableSpecTime, step 5\n";
  return alphaLow*eVarLow + alphaUpp*eVarUpp;
}


void PrintGribMessage(std::string const& FileName, MyMatrix<double> const& TheField, GRIB_MessageInfo const& eMesg, GridArray const& GrdArr)
{
  FILE* of = NULL;
  long step=0;
  grib_handle *h = NULL;
