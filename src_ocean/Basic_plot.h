...
std::string FinalFile(bool const& InPlaceRun, std::string const& TargetFile, std::string const& eFile)
{
  if (InPlaceRun) {
...
std::string FinalFileInScript(bool const& InPlaceRun, std::string const& eFile)
{
  if (InPlaceRun)
		  ...
		  PermanentInfoDrawing const& ePerm)
{
  ...
  std::string eFileNC=FinalFile(InPlaceRun, TargetFile, ePerm.PrefixTemp.str() + "DataScatter_" + VarNameAB_file + ".nc");
  std::string eFileNCL=FinalFile(InPlaceRun, TargetFile, ePerm.PrefixTemp.str() + "ScriptScatter_" + VarNameAB_file + ".ncl");
  ...
  OUTncl << "begin\n";
  OUTncl << "  f = addfile(\"" << FinalFileInScript(InPlaceRun, eFileNC) << "\", \"r\")\n";
  OUTncl << "  X = f->X2D\n";
  OUTncl << "  Y = f->Y2D\n";
  ...
  OUTncl << "  maxValue=2\n";
  OUTncl << "  wks  = gsn_open_wks (\"" << eReal.eExtensionReal << "\",\"" << FinalFileInScript(InPlaceRun, FILE_RemoveExtension(TargetFile)) << "\")\n";
  ...
  OUTncl << "  vres1@cnFillOn             = " << NCL_bool(ePerm.eDrawArr.cnFillOn) << "   ; turn on color for contours\n";
  ...
  if (eDrawScatter.AddStatMeasModel) {
    ...
    T_statString eStatStr=ComputeStatisticString_from_Statistics(eStat, optStatStr);
    std::string strWrite="m=" + eStatStr.strSlope + " c=" + eStatStr.strCorrelation + " s=" + eStatStr.strScatterIndex;
    OUTncl << "  txresB             = True\n";
  ...
  OUTncl.close();
  //
  TripleNCL eTripl{eReal.eProgReal, TargetFile, eFileNC, eFileNCL, ePerm.eBChoice};
  GeneralType eGen(eTripl);
  eCall.SubmitJob(eGen);
}


void ADD_LISTLINESEGMENT_NC(netCDF::NcFile & dataFile,
  ...
  if (xi > 1) {
	...
	for (int j=0; j<xi; j++) {
	  ...
	  if (GrdArr.GrdArrRho.MSK(i,j) == 1 || !ApplyCritValue) {
	    ...
	    //	  std::cerr << "Assignation to eCritValue\n";
	    valU[idx]=eCritValue;
	    valV[idx]=eCritValue;
	    valF[idx]=eCritValue;
	  }
		 ...
		 PermanentInfoDrawing const& ePerm)
{
  ...
  OUTncl << "  vres1@cnLineLabelsOn       = " << NCL_bool(eDrawArr.cnLineLabelsOn) << "   ; turn off contour line labels\n";
	       ...
	       PermanentInfoDrawing const& ePerm)
{
  ...
  if (IsFE == 0)
    return;
  bool InPlaceRun=ePerm.eBChoice.InPlaceRun;
  std::string eFileNC=FinalFile(InPlaceRun, TargetFile, ePerm.PrefixTemp.str() + "mesh.nc");
  std::string eFileNCL=FinalFile(InPlaceRun, TargetFile, ePerm.PrefixTemp.str() + "mesh.ncl");
  //  std::cerr << "eFileNC=" << eFileNC << "\n";
			 ...
			 PermanentInfoDrawing const& ePerm)
{
  ...
  double midLON=(minLON + maxLON) / double(2);
  double minLAT=eDrawArr.eQuadFrame.MinLat;
  double maxLAT=eDrawArr.eQuadFrame.MaxLat;
  double midLAT=(minLAT + maxLAT) / double(2);
  std::cerr << "eDrawArr.eQuadFrame=" << eDrawArr.eQuadFrame.MinLon << " / " << eDrawArr.eQuadFrame.MaxLon << " / " << eDrawArr.eQuadFrame.MinLat << " / " << eDrawArr.eQuadFrame.MaxLat << "\n";
  ...
  int nbLevelSpa=eDrawArr.nbLevelSpa;
  double deltaVal=(eMaxVal - eMinVal) / double(nbLevelSpa-1);
  ...
  if (eDrawArr.ListLineSegment.size() > 0) {
    ...
    os << "    map.drawgreatcircle(lon1,lat1,lon2,lat2,linewidth=2,color=\'b\')\n";
  }
  os << "\n";
		     ...
		     PermanentInfoDrawing const& ePerm)
{
  ...
  OUTncl << "  res2@cnFillOn             = " << NCL_bool(eDrawArr.cnFillOn) << "   ; turn on color for contours\n";
  OUTncl << "  res2@cnLinesOn            = " << NCL_bool(eDrawArr.cnLinesOn) << "\n";
  OUTncl << "  res2@cnLineLabelsOn       = " << NCL_bool(eDrawArr.cnLineLabelsOn) << "   ; turn off contour line labels\n";
		 ...
		 PermanentInfoDrawing const& ePerm)
{
  if (!IsEqualSizeMatrices(GrdArr.GrdArrRho.LON, eRecVar.F)) {
    std::cerr << "Now GrdArr.GrdArrRho.LON and eRecVar.F should have the same size\n";
    std::cerr << "|GrdArr.GrdArrRho.LON|=" << GrdArr.GrdArrRho.LON.rows() << " / " << GrdArr.GrdArrRho.LON.cols() << "\n";
    std::cerr << "           |eRecVar.F|=" << eRecVar.F.rows() << " / " << eRecVar.F.cols() << "\n";
    throw TerminalException{1};
  }
  std::string eProg=ePerm.ChoiceProgram.at("Pcolor_method");
  //  std::cerr << "PLOT_PCOLOR eProg=" << eProg << " eRecVar.RecS.strAll=" << eRecVar.RecS.strAll << "\n";
  bool IsMatch=false;
  if (eProg == "ncl") {
  ...
  if (!IsMatch) {
    std::cerr << "eProg = " << eProg << " while allowed values are python and ncl\n";
    std::cerr << "If you want the crop, use pngcrop or pdfcrop in the type instead of png, pdf\n";
    throw TerminalException{1};
  }
}


void PLOT_PCOLOR_OR_QUIVER(std::string const& FileName,
			   GridArray const& GrdArr,
			   DrawArr const& eDrawArr,
			   RecVar const& eRecVar,
			   NCLcaller<GeneralType> & eCall,
		     ...
		     DrawLinesArr const& eDrawArr)
{
  ...
  if (eDrawArr.IsTimeSeries) {
    ...
    for (int i=0; i<nbEntry; i++) {
      ...
      double FirstDayYearNext=DATE_ConvertSix2mjd({eVect[0]+1, 1, 1, 0,0,0});
      double lenYear=FirstDayYearNext - FirstDayYear;
      double eX = double(eVect[0]) + (eMJD - FirstDayYear) / lenYear;
      valYear[i]=eX;
    }
		    ...
		    PermanentInfoDrawing const& ePerm)
{
  ...
  std::string TargetFile=FileName + "_storsave." + eReal.eExtensionReal;
  bool InPlaceRun=ePerm.eBChoice.InPlaceRun;
  std::string eFileNC=FinalFile(InPlaceRun, TargetFile, ePerm.PrefixTemp.str() + "DataLines_" + eDrawArr.VarName + ".nc");
  std::string eFileNCL=FinalFile(InPlaceRun, TargetFile, ePerm.PrefixTemp.str() + "ScriptLines_" + eDrawArr.VarName + ".ncl");
  ...
  std::vector<std::string> ListColors={"black", "red", "blue", "purple", "green", "cyan", "azure", "marron", "aquamarine", "violet", "DarkRed"};
  ...
  if (eDrawArr.DoExplicitLabel) {
    ...
    for (int iLabel=0; iLabel<nbLabel; iLabel++) {
      double pos_d = double(iLabel) * double(nbEnt-1) / double(nbLabel-1);
      int pos_i;
      NearestInteger_double_int(pos_d, pos_i);
      pos_i = std::max(0, pos_i);
      pos_i = std::min(nbEnt-1, pos_i);
      std::cerr << "iLabel=" << iLabel << " nbLabel=" << nbLabel << " nbEnt=" << nbEnt << " pos_d=" << pos_d << " pos_i=" << pos_i << "\n";
      if (iLabel > 0) {
	strDay += ", ";
	strLabel += ", ";
      }
	...
	pos_i = nbEnt-1;
      ...
      strLabel += "\"" + DATE_ConvertSix2string_style(eDate, eDrawArr.StyleDate) + "\"";
    }
		       ...
		       PermanentInfoDrawing const& ePerm)
{
  ...
  bool InPlaceRun=ePerm.eBChoice.InPlaceRun;
  std::string eFileNC=FinalFile(InPlaceRun, TargetFile, ePerm.PrefixTemp.str() + "DataLines_" + eDrawArr.VarName + ".nc");
  std::string eFilePY=FinalFile(InPlaceRun, TargetFile, ePerm.PrefixTemp.str() + "ScriptLines_" + eDrawArr.VarName + ".py");
  LINES_DEFINE_NC(eFileNC, eDrawArr);
  int nbArr=eDrawArr.ListListVect.size();
  //
  if (nbArr != 1) {
  ...
  os << "    ListTime[i] = datetime.datetime(eYear, eMonth, eDay, eHour, eMin, eSec)\n";
