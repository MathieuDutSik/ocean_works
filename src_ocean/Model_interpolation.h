...
MyMatrix<double> SingleInterpolationOfField_2D(SingleArrayInterpolation const& eInterp, MyMatrix<double> const& Fin)
{
  ...
  //  std::cerr << "eInterp.SpMat(rows/cols)=" << eInterp.SpMat.rows() << " / " << eInterp.SpMat.cols() << "\n";
...
SingleArrayInterpolation GetSingleArrayInterpolationTrivialCase(GridArray const& GrdArrOut, GridArray const& GrdArrIn)
{
  ...
  auto ComputeInterpolationArray=[&](MyMatrix<int> const& INE, MyMatrix<double> const& LON, MyMatrix<double> const& LAT, int const& nbNodeInput) -> SingleArrayInterpolation {
    int nbTrig=INE.rows();
  ...
  if (GrdArrIn.IsFE == 1) {
    ...
    for (int k=0; k<SpMat.outerSize(); k++)
      for (typename MySparseMatrix<double>::InnerIterator it(SpMat,k); it; ++it) {
...
SingleArrayInterpolation ConvertToArrayInt(int const& eta_out, int const& xi_out, int const& eta_in, int const& xi_in, std::vector<int> const& LEta, std::vector<int> const& LXi, std::vector<SingleRecInterp> const& LRec, GridArray const& GrdArrOut, ARVDtyp const& ARVDin)
{
  typedef Eigen::Triplet<double> T2;
...
RecVar INTERPOL_GetHatFunction(GridArray const& GrdArrOut, double const& TheSize)
{
  ...
  if (GrdArrOut.IsFE == 1) {
	...
	std::vector<double> LValY{double(1), double(j)/TheSize, double(xi_rho-1-j)/TheSize};
...
MyMatrix<double> HatFunctionFromMask(MyMatrix<int> const& MSKinput, GridArray const& GrdArr, GraphSparseImmutable const& eGR, int const& SpongeSize)
{
  ...
  for (int iVal=2; iVal<SpongeSize; iVal++) {
	...
	if (MSKinput(i,j) == 1 && TheMSKwork(i,j) == 0) {
	  int IsNeighLower=0;
	  std::vector<Pair> LPair=GetListAdjacent(i,j);
	  //	  std::cerr << "After GetListAdjacent i=" << i << " j=" << j << " |LPair|=" << LPair.size() << "\n";
	  for (auto & ePair : LPair)
...
std::string ConvertTypename(std::string const& name1, std::string const& name2)
{
...
std::string ROMS_Surface_NetcdfInitialize_SingleVar(netCDF::NcFile & dataFile, bool const& ConstantDefinition, bool const& PutLonLatAngArray, bool const& IsRegrid, GridArray const& GrdArr, std::string const& eVarName)
{
  int eta_rho=GrdArr.GrdArrRho.LON.rows();
  int xi_rho =GrdArr.GrdArrRho.LON.cols();
  TotalArrGetData TotalArr;
  ...
  if (strTime_ROMS == "unset") {
    std::cerr << "We require the variable to be one of the standard ROMS supported ones.\n";
...
std::vector<ROMS_NC_VarInfo> ROMS_Surface_NetcdfInitialize(std::string const& eFileNC, bool const& IsRegrid, bool const& SingleFile, GridArray const& GrdArr, std::vector<std::string> const& ListVarName)
{
  ...
  for (int iVar=0; iVar<nbVar; iVar++) {
    ...
    std::string strTime=ROMS_Surface_NetcdfInitialize_SingleVar(dataFile, ConstantDefinition, PutLonLatAngArray, IsRegrid, GrdArr, eVarName);
...
void ROMS_BOUND_NetcdfInitialize(std::string const& eFileNC, GridArray const& GrdArr, std::vector<std::string> const& ListSides)
{
  ...
  for (auto & eStr : ListSides) {
    if (eStr != "South" && eStr != "North" && eStr != "West" && eStr != "East") {
  ...
  if (ListSides.size() == 0) {
    ...
    std::cerr << "e.g. ListSides = \"South\", \"North\", \"West\", \"East\",    \n";
    throw TerminalException{1};
  }
  double RefTimeROMS=DATE_ConvertSix2mjd({1968, 05, 23, 0, 0, 0});
  int eta_rho=GrdArr.GrdArrRho.LON.rows();
  ...
  if (posEast != -1) {
    ...
    netCDF::NcVar eVAR4=dataFile.addVar("ubar_east", "float", {strV2dTime, strEtaU});
    netCDF::NcVar eVAR5=dataFile.addVar("vbar_east", "float", {strV2dTime, strEtaV});
    netCDF::NcVar eVAR6=dataFile.addVar("u_east", "float", {strV3dTime, strSRho, strEtaU});
  ...
  if (posWest != -1) {
    ...
    netCDF::NcVar eVAR4=dataFile.addVar("ubar_west", "float", {strV2dTime, strEtaU});
    netCDF::NcVar eVAR5=dataFile.addVar("vbar_west", "float", {strV2dTime, strEtaV});
    netCDF::NcVar eVAR6=dataFile.addVar("u_west", "float", {strV3dTime, strSRho, strEtaU});
  ...
  if (posNorth != -1) {
    ...
    netCDF::NcVar eVAR4=dataFile.addVar("ubar_north", "float", {strV2dTime, strXiU});
    netCDF::NcVar eVAR5=dataFile.addVar("vbar_north", "float", {strV2dTime, strXiV});
    netCDF::NcVar eVAR6=dataFile.addVar("u_north", "float", {strV3dTime, strSRho, strXiU});
  ...
  if (posSouth != -1) {
    ...
    netCDF::NcVar eVAR4=dataFile.addVar("ubar_south", "float", {strV2dTime, strXiU});
    netCDF::NcVar eVAR5=dataFile.addVar("vbar_south", "float", {strV2dTime, strXiV});
    netCDF::NcVar eVAR6=dataFile.addVar("u_south", "float", {strV3dTime, strSRho, strXiU});
    netCDF::NcVar eVAR7=dataFile.addVar("v_south", "float", {strV3dTime, strSRho, strXiV});
  }
  //
...
void ROMS_BOUND_NetcdfAppend_Kernel(std::string const& eFileNC, ROMSstate const& eState, std::vector<std::string> const& ListSides, int const& pos)
{
  ...
  if (posSouth != -1) {
    ...
    for (int i=0; i<xi_rho; i++)
      A[i]=float(eState.ZETA(0, i));
    netCDF::NcVar eVar1=dataFile.getVar("zeta_south");
    eVar1.putVar(start, count, A);
    delete [] A;
    //
    ...
    idx=0;
    for (int j=0; j<xi_u; j++) {
      A[idx]=float(eState.Ubar(0, j));
      idx++;
    }
    netCDF::NcVar eVar6=dataFile.getVar("ubar_south");
    eVar6.putVar(start, count, A);
    delete [] A;
    //
    A=new float[xi_v];
    start={size_t(pos),0};
    ...
    idx=0;
    for (int j=0; j<xi_v; j++) {
      A[idx]=float(eState.Vbar(0, j));
      idx++;
    }
    netCDF::NcVar eVar7=dataFile.getVar("vbar_south");
    eVar7.putVar(start, count, A);
    delete [] A;
    //
  }
  if (posNorth != -1) {
    ...
    count={1, size_t(xi_rho)};
    for (int i=0; i<xi_rho; i++)
      A[i]=float(eState.ZETA(eta_rho-1, i));
    ...
    for (int i=0; i<s_rho; i++)
      for (int j=0; j<xi_rho; j++) {
	A[idx]=float(eState.Temp(i, eta_rho-1, j));
	idx++;
      }
    ...
    for (int i=0; i<s_rho; i++)
      for (int j=0; j<xi_rho; j++) {
	A[idx]=float(eState.Salt(i, eta_rho-1, j));
	idx++;
      }
    ...
    for (int i=0; i<s_rho; i++)
      for (int j=0; j<xi_u; j++) {
	A[idx]=float(eState.U(i, eta_u-1, j));
	idx++;
      }
    ...
    for (int i=0; i<s_rho; i++)
      for (int j=0; j<xi_v; j++) {
	A[idx]=float(eState.V(i, eta_v-1, j));
	idx++;
      }
    ...
    idx=0;
    for (int j=0; j<xi_u; j++) {
      A[idx]=float(eState.Ubar(eta_u-1, j));
      idx++;
    }
    ...
    idx=0;
    for (int j=0; j<xi_v; j++) {
      A[idx]=float(eState.Vbar(eta_v-1, j));
      idx++;
    }
  ...
  if (posEast != -1) {
    ...
    for (int i=0; i<eta_rho; i++)
      A[i]=float(eState.ZETA(i, xi_rho-1));
	...
	A[idx]=float(eState.Temp(i, j, xi_rho-1));
	idx++;
      }
    ...
    for (int i=0; i<s_rho; i++)
      for (int j=0; j<eta_rho; j++) {
	A[idx]=float(eState.Salt(i, j, xi_rho-1));
	idx++;
      }
    ...
    delete [] A;
    //
    A=new float[s_rho*eta_u];
    start={size_t(pos),0,0};
    count={1, size_t(s_rho), size_t(eta_u)};
    idx=0;
    for (int i=0; i<s_rho; i++)
      for (int j=0; j<eta_u; j++) {
	A[idx]=float(eState.U(i, j, xi_u-1));
	idx++;
      }
    ...
    delete [] A;
    //
    A=new float[s_rho*eta_v];
    start={size_t(pos),0,0};
    count={1, size_t(s_rho), size_t(eta_v)};
    idx=0;
    for (int i=0; i<s_rho; i++)
      for (int j=0; j<eta_v; j++) {
	A[idx]=float(eState.V(i, j, xi_v-1));
	idx++;
      }
    ...
    idx=0;
    for (int j=0; j<eta_u; j++) {
      A[idx]=float(eState.Ubar(j, xi_u-1));
      idx++;
    }
    ...
    idx=0;
    for (int j=0; j<eta_v; j++) {
      A[idx]=float(eState.Vbar(j, xi_v-1));
      idx++;
    }
...
void INTERPOL_NetcdfAppendVarName(std::string const& eFileNC, GridArray const& GrdArr, std::vector<RecVar> const& ListRecVar, RecTime & eRec)
{
  ...
  for (auto & eRecVar : ListRecVar) {
	...
	std::string nameU = ConvertTypename(eRecVar.RecS.nameU, eRecVar.RecS.FullVarName);
	std::string nameV = ConvertTypename(eRecVar.RecS.nameV, eRecVar.RecS.FullVarName);
	netCDF::NcVar eVar_U=dataFile.getVar(nameU);
	netCDF::NcVar eVar_V=dataFile.getVar(nameV);
	for (int i=0; i<nbWet; i++)
...
void WaveWatch_WriteData(GridArray const& GrdArrOut, std::vector<RecVar> const& ListRecVar, int & WWIII_nbWritten)
{
  if (ListRecVar.size() == 0) {
  ...
  if (WWIII_posWind10 == -1 && WWIII_posSurfCurr == -1 && WWIII_posZetaOcean == -1) {
    std::cerr << "It is erroneous to call the WaveWatch III output\n";
    std::cerr << "if neither WIND10 nor surface current nor free surface are not selected\n";
    throw TerminalException{1};
  }
  //
  // 
  ...
  if (WWIII_posWind10 != -1) {
    float *Uvect, *Vvect;
    Uvect=new float[nx*ny];
    Vvect=new float[nx*ny];
    for (int i=0; i<nx; i++)
      for (int j=0; j<ny; j++) {
    ...
    write_wavewatch_entry_two_field_("wind.ww3", TFN, &nx, &ny, Uvect, Vvect);
  ...
  if (WWIII_posSurfCurr != -1) {
    ...
    write_wavewatch_entry_two_field_("current.ww3", TFN, &nx, &ny, Uvect, Vvect);
...
void INTERPOL_NetcdfOutput(GridArray const& GrdArrOut, std::vector<RecVar> const& ListRecVar, FullNamelist const& eFull, int const& eMult, recNetcdfOutput & recNO)
{
  ...
  std::string HisPrefixOut=eBlNETCDF_STANDARD.ListStringValues.at("HisPrefixOut");
  std::vector<std::string> ListVarName;
  double eTimeDay=0;
  for (auto & eRecVar : ListRecVar) {
    ListVarName.push_back(eRecVar.RecS.VarName1);
...
void INTERPOL_GribAppendVarName(std::string const& eFileGrib, GridArray const& GrdArrOut, std::vector<RecVar> const& ListRecVar)
{
  ...
  GRIB_CHECK(grib_set_double(h, "longitudeOfFirstGridPointInDegrees", lonFirst), 0);
  GRIB_CHECK(grib_set_double(h, "latitudeOfFirstGridPoint", latFirst), 0);
  GRIB_CHECK(grib_set_double(h, "latitudeOfFirstGridPointInDegrees", latFirst), 0);
  GRIB_CHECK(grib_set_double(h, "longitudeOfLastGridPoint", lonLast), 0);
  GRIB_CHECK(grib_set_double(h, "longitudeOfLastGridPointInDegrees", lonLast), 0);
  GRIB_CHECK(grib_set_double(h, "latitudeOfLastGridPoint", latLast), 0);
  GRIB_CHECK(grib_set_double(h, "latitudeOfLastGridPointInDegrees", latLast), 0);
  double *A;
  int siz=Ni * Nj;
...
void INTERPOL_GribOutput(GridArray const& GrdArrOut, std::vector<RecVar> const& ListRecVar, FullNamelist const& eFull, int const& eMult, recGribOutput & recGO)
{
  ...
  std::string HisPrefixOut=eBlGRIB_STANDARD.ListStringValues.at("HisPrefixOut");
  std::vector<std::string> ListVarName;
