...
void CheckingGridPointStatus(std::vector<GridArray> const &ListGrdArr,
  ...
  for (int iGrid = 0; iGrid < nbGrid; iGrid++)
    ListListBelong[iGrid] =
        DetermineBelonging_ListXY(ListGrdArr[iGrid], ListXY);
  int nbWrongBuoy = 0;
...
void BUOY_Plot(FullNamelist const &eFull) {
  ...
  for (int iGrid = 0; iGrid < nbGrid; iGrid++)
    ListRec[iGrid] = {
        ComputeArrayInterpolation_ListXY(ListGrdArr[iGrid], ListXY), {}};
  //
  ...
  for (int iBuoy = 0; iBuoy < nbBuoy; iBuoy++) {
    ...
    std::vector<double> ListTimeBuoy =
        NC_ReadTimeFromFile(eBuoyFileName, "TIME");
    ...
    for (int u = 0; u < diffNb; u++)
      std::cerr << " ";
    std::cerr << " minTimePres=" << minTimePres
              << " maxTimePres=" << maxTimePres << "\n";
  }
  //
  ...
  for (auto &eVarName : ListVarOut) {
    ...
    for (int iBuoy = 0; iBuoy < nbBuoy; iBuoy++) {
      ...
      std::cerr << "eBuoyFileName = " << eBuoyFileName
                << "    eVarCall = " << eVarCall << "\n";
      std::vector<double> ListTimeBuoy =
          NC_ReadTimeFromFile(eBuoyFileName, "TIME");
      std::cerr << "ListTimeBuoy(min/max)=" << VectorMin(ListTimeBuoy) << " / "
                << VectorMax(ListTimeBuoy) << "\n";
      MyVector<double> ListVal;
      std::vector<size_t> LDim =
          NC_ReadVariable_listdim_file(eBuoyFileName, eVarCall);
      if (LDim.size() == 2) {
        MyMatrix<double> eMat = NC_Read2Dvariable(eBuoyFileName, eVarCall);
        int nbRow = eMat.rows();
        int nbCol = eMat.cols();
        std::cerr << "eMat rows = " << eMat.rows()
                  << "    cols = " << eMat.cols() << "\n";
        int iColFind = -1;
        int nbFind = 0;
      ...
      std::cerr << "|ListTimeBuoy| = " << ListTimeBuoy.size()
                << "     |ListVal| = " << ListVal.size() << "\n";
      if (int(ListTimeBuoy.size()) != int(ListVal.size())) {
        std::cerr << "ListTimeBuoy and ListVal are of different size\n";
        throw TerminalException{1};
      }
      std::cerr << "ListVal(min/max)=" << ListVal.minCoeff() << " / "
                << ListVal.maxCoeff() << "\n";
      ...
      for (int iTime = 0; iTime < nbTime; iTime++) {
        ...
        if (deltaTime < MaximumLengthInterpolationIntervalDay) {
          ...
          if (ListVal(eInterp.iTimeLow) > MissingValueDetect &&
              ListVal(eInterp.iTimeUpp) > MissingValueDetect) {
            std::cerr << "Pass the MissingValue test\n";
            double eVal = eInterp.alphaLow * ListVal(eInterp.iTimeLow) +
                          eInterp.alphaUpp * ListVal(eInterp.iTimeUpp);
      ...
      for (int uTimeSel = 0; uTimeSel < nbTimeSel; uTimeSel++) {
        ...
        if (uTimeSel > 0) {
          ...
          if (ListInterpVal(eTimePrev) <
              ListInterpVal(eTimeSel) - MaxAllowedDecrease)
            IsCorrect = false;
        }
        if (uTimeSel < nbTimeSel - 1) {
          int eTimeNext = ListITime[uTimeSel + 1];
          if (ListInterpVal(eTimeNext) <
              ListInterpVal(eTimeSel) - MaxAllowedDecrease)
            IsCorrect = false;
        }
        if (!IsCorrect)
    ...
    for (int iGrid = 0; iGrid < nbGrid; iGrid++)
      for (int iTime = 0; iTime < nbTime; iTime++) {
        ...
        RecS = eRecVar.RecS;
        RecVar fRecVar =
            INTERPOL_SingleRecVarInterpolation(ListRec[iGrid], eRecVar);
        for (int iBuoy = 0; iBuoy < nbBuoy; iBuoy++) {
    ...
    for (int iBuoy = 0; iBuoy < nbBuoy; iBuoy++) {
      ...
      for (int iTime = 0; iTime < nbTime; iTime++) {
        ...
        //
        std::string strPres =
            DATE_ConvertMjd2mystringPres(ListTime_vect(iTime));
      ...
      if (DoLinePlot) {
        for (int iBlock = 0; iBlock < nbBlock; iBlock++) {
          ...
          eDrawArr.TitleStr =
              "Time series of " + eVarName + " for buoy " + eBuoyFileNamePlot;
          ...
          eDrawArr.VarName = eVarName + "_" + std::to_string(iBuoy + 1) + "_" +
                             std::to_string(iBlock);
          ...
          //
          std::string FileName = ePerm.eDir + "TimeSeries_iBuoy" +
                                 std::to_string(iBuoy + 1) + "_iBlock" +
                                 std::to_string(iBlock) + "_" + eVarName;
      ...
      for (int iGrid = 0; iGrid < nbGrid; iGrid++) {
        ...
        int len = eVectA.size();
        for (int u = 0; u < len; u++) {
          std::cerr << "u=" << u << " / " << len << " eVectA=" << eVectA(u)
                    << " eVectB=" << eVectB(u) << "\n";
        }
        T_stat eStat = ComputeStatistics_MyVector(eVectA, eVectB);
        std::string eName = "iBuoy" + std::to_string(iBuoy) + "_iGrid" +
                            std::to_string(iGrid) + "_Global";
        Print_Down_Statistics(std::cerr, eName, eStat);
        os << "eVarName=" << eVarName << " iBuoy=" << iBuoy
           << " iGrid=" << iGrid << "\n";
        Print_Down_Statistics(os, eName, eStat);
        if (DoScatterPlot) {
          DrawScatterArr eDrw;
          eDrw.VarNameAB_file =
              "Scatter_" + std::to_string(iBuoy + 1) + "_" + eVarName;
          eDrw.DoTitle = true;
...
template <typename T>
std::vector<T> ReplicateInformation(std::vector<T> const &V,
                                    size_t const &n_elt,
...
void PointOutputPlot(FullNamelist const &eFull) {
  ...
  ListTotalArr =
      ReplicateInformation(ListTotalArr, nbGridVar_t, "ListTotalArr");
  ...
  for (int iRegion = 0; iRegion < nbRegion; iRegion++) {
    ...
    std::vector<std::string> LStr_lon =
        STRING_Split(ListAverageRegionLon[iRegion], " ");
    std::vector<std::string> LStr_lat =
        STRING_Split(ListAverageRegionLat[iRegion], " ");
  ...
  for (int iGridVar = 0; iGridVar < nbGridVar; iGridVar++)
    ListRecRegAve[iGridVar] = ComputeArrayRegionAveraging_ListPolygon(
        ListGrdArr[iGridVar], ListRegions);
  ...
  //
  std::vector<int> ListPos = DivideListPosition(nbTime, nbBlock);
  std::vector<std::vector<MyVector<double>>> ListListVect(
      nbBuoy + nbRegion,
      std::vector<MyVector<double>>(nbGridVar, MyVector<double>(nbTime)));
  RecSymbolic RecS;
  for (int iGridVar = 0; iGridVar < nbGridVar; iGridVar++) {
    ...
    for (int iTime = 0; iTime < nbTime; iTime++) {
      ...
      std::cerr << "iTime=" << iTime << " / " << nbTime << "\n";
      RecVar eRecVar = ModelSpecificVarSpecificTimeGeneral(
          ListTotalArr[iGridVar], eVarName, eQuery, ePlotBound);
      RecS = eRecVar.RecS;
      std::string eUnit = RecS.Unit;
      RecVar fRecVar =
          INTERPOL_SingleRecVarInterpolation(ListRec[iGridVar], eRecVar);
      for (int iBuoy = 0; iBuoy < nbBuoy; iBuoy++) {
        double eVal = fRecVar.F(iBuoy, 0);
        ListListVect[iBuoy][iGridVar](iTime) = eVal;
      }
      RecVar gRecVar =
          REGAVE_SingleRecVarAveraging(ListRecRegAve[iGridVar], eRecVar);
      if (gRecVar.F.cols() != 1 || gRecVar.F.rows() != nbRegion) {
        std::cerr << "The dimension of gRecVar.F is not adequate. Maybe wrong "
                     "variable was called\n";
        throw TerminalException{1};
      }
      std::cerr << "|gRecVar.F|=" << gRecVar.F.rows() << " / "
                << gRecVar.F.cols() << "\n";
      for (int iRegion = 0; iRegion < nbRegion; iRegion++) {
  ...
  if (DoCsvFile) {
    for (int iBuoyReg = 0; iBuoyReg < nbBuoy + nbRegion; iBuoyReg++) {
      std::cerr << "iBuoyReg=" << iBuoyReg << " nbBuoy=" << nbBuoy
                << " nbRegion=" << nbRegion << "\n";
      std::string OutputFile =
          PicPrefix + "/csv_file" + std::to_string(iBuoyReg + 1) + ".csv";
      ...
      for (int iGridVar = 0; iGridVar < nbGridVar; iGridVar++)
        ListRecSymb[iGridVar] =
            RetrieveTrivialRecVar(ListVarName[iGridVar]).RecS;
      //
      os << "Long name";
      for (int iGridVar = 0; iGridVar < nbGridVar; iGridVar++)
  ...
  if (DoLinePlot) {
    for (int iBuoyReg = 0; iBuoyReg < nbBuoy + nbRegion; iBuoyReg++) {
      ...
      for (int iBlock = 0; iBlock < nbBlock; iBlock++) {
        ...
        eDrawArr.StyleDate = StyleDate;
        eDrawArr.VarName =
            std::to_string(iBuoyReg + 1) + "_" + std::to_string(iBlock);
        ...
        //
        std::string FileName = ePerm.eDir + "TimeSeries_iBuoyReg" +
                               std::to_string(iBuoyReg + 1) + "_iBlock" +
                               std::to_string(iBlock);
        LINES_PLOT(FileName, eDrawArr, eCall, ePerm);
      }
    }
