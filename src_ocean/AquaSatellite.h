...
void AquaDownloading(FullNamelist const& eFull)
{
  ...
  std::vector<std::string> ListVarName=BlAQUA.ListListStringValues.at("ListVarName");
  std::string CoastlineResolution_Lines=BlAQUA.ListStringValues.at("CoastlineResolution_Lines");
  int GridSubsample_lines_row=BlAQUA.ListIntValues.at("GridSubsample_lines_row");
  int GridSubsample_lines_col=BlAQUA.ListIntValues.at("GridSubsample_lines_col");
  bool GlobalGrid_lines=BlAQUA.ListBoolValues.at("GlobalGrid_lines");
  bool PlotFullGeographicExtent=BlAQUA.ListBoolValues.at("PlotFullGeographicExtent");
  bool SetMaximumRangeDynamic=BlAQUA.ListBoolValues.at("SetMaximumRangeDynamic");
  std::vector<std::string> ListEmailAddress=BlPROC.ListListStringValues.at("ListEmailAddress");
  ...
  auto IsGeographicallyCorrect=[&](DATAaqua const& eDATA) -> bool {
    ...
    return IsContainedInDomain(LONcornerDom, LATcornerDom, LONcornerSat, LATcornerSat);
  };
  ...
  auto DownLink=[&](std::string const& eLink) -> std::string {
    ...
    while(true) {
      std::string eComm="(cd " + TheDir + " && wget " + eLink + " > " + tmpStdout + ")";
  ...
  auto CreateStorageDirectory=[&](std::string const& DataFile) -> void {
    ...
    int eMonth=LTime[1];
    int eDay=LTime[2];
    std::string StorDir = StorePrefix + StringNumber(eYear,4) + "/" + StringNumber(eMonth,2) + "/" + StringNumber(eDay,2) + "/";
  ...
  auto GetStorageFileName=[&](std::string const& DataFile) -> std::string {
    ...
    int eMonth=LTime[1];
    int eDay=LTime[2];
    return StorePrefix + StringNumber(eYear,4) + "/" + StringNumber(eMonth,2) + "/" + StringNumber(eDay,2) + "/" + CoreString + ".nc";
  };
  auto GetNameUselessStorage=[&](std::string const& DataFile) -> std::string {
    ...
    return StorePrefix + StringNumber(eYear,4) + "/" + StringNumber(eMonth,2) + "/" + StringNumber(eDay,2) + "/ListUselessFiles";
  };
  auto GetListUselessFiles=[&](std::string const& DataFile) -> std::vector<std::string> {
  ...
  auto StorageOperation=[&](std::string const& DataFile, bool const& testIncl) -> void {
    if (testIncl) {
      std::string StorageFile = GetStorageFileName(DataFile);
      if (!IsExistingFile(StorageFile))  {
	CreateStorageDirectory(DataFile);
  ...
  auto FullyTreatFile=[&](std::string const& DataFile) -> void {
    ...
    if (eData.LON.minCoeff() < -200 || eData.LON.maxCoeff() > 200 || eData.LAT.minCoeff() < -100 || eData.LAT.maxCoeff() > 100) {
      std::cerr << "DataFile=" << DataFile << "\n";
      std::cerr << "|eData.LON| = " << StringSizeMatrix(eData.LON) << " " << MinMaxMatrix(eData.LON) << "\n";
      std::cerr << "|eData.LAT| = " << StringSizeMatrix(eData.LAT) << " " << MinMaxMatrix(eData.LAT) << "\n";
    ...
    MyMatrix<double> LONred=MatrixSubsample(eData.LON, GridSubsample_lines_row, GridSubsample_lines_col);
    MyMatrix<double> LATred=MatrixSubsample(eData.LAT, GridSubsample_lines_row, GridSubsample_lines_col);
    ...
    if (PlotSatelliteCover) {
      ...
      std::string FileName=ePerm.eDir + "Lines_" + CoreString + "_-_" + strTimeFileFirst + "_" + strTimeFileLast;
	...
	RecVar eRecVar=ModelSpecificVarSpecificTime(TotalArr, eVarName, midTime);
	...
	std::string FileName=ePerm.eDir + "Pcolor_" + eVarName + "_" + strTimeFileFirst + "_" + strTimeFileLast;
	std::cerr << "FileName = " << FileName << "\n";
	//
	QuadArray eQuadPC;
	...
	if (SetMaximumRangeDynamic) {
	  ...
	  nbVal=ListVal.size();
	  std::cerr << "nbRow=" << nbRow << " nbCol=" << nbCol << " nbVal=" << nbVal << "\n";
	  if (nbVal > 0) {
	...
	std::cerr << "STRALL 2 : eVarName=" << eVarName << " eRecVar.strAll=" << eRecVar.RecS.strAll << "\n";
	...
	Text += "\n";
	Text += "please find in attachment picture downloaded from the Aqua satellite\n";
	Text += "\n";
	Text += "  Mathieu\n";
	std::string FromEmail="mathieu.dutour@gmail.com";
	for (auto & DestEmail : ListEmailAddress) {
	  SendMailOper eSendOper{DestEmail, FromEmail, Subject, Text, ListPictureFile};
  ...
  auto DownloadLinkAndTreat_if_OC_and_undone=[&](std::string const& eLink) -> void {
  ...
  if (MethodOperation == "UseFixedList") {
    DidSomething=true;
    std::vector<std::string> ListNakedFile=BlPROC.ListListStringValues.at("ListNakedFile");
    std::string GetfileLinkPrefix=BlPROC.ListStringValues.at("GetfileLinkPrefix");
    std::string FirstChar=GetfileLinkPrefix.substr(0,1);
    if (FirstChar != "/" && FirstChar != "h") {
      std::cerr << "First character should be / or h, i.e. full path to file or https\n";
      throw TerminalException{1};
    }
    if (FirstChar == "h") {
  ...
  if (MethodOperation == "FixedPeriod") {
    ...
    std::vector<std::string> ListLink=GetAquaListLinkDownload_Specified(eFull, MjdFirst, MjdLast);
    for (auto & eLink : ListLink)
      DownloadLinkAndTreat_if_OC_and_undone(eLink);
  }
  if (!DidSomething) {
    std::cerr << "We did not select a correct method of operation\n";
    std::cerr << "Allowed values for MethodOperation: FixedPeriod, ContinuousOperation, UseFixedList\n";
    throw TerminalException{1};
  }
}
