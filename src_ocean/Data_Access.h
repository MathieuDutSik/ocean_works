...
std::vector<OneArrayRadarMeas> ReadRadarFile(std::string const& eFileName)
{
  ...
  int pos=-1;
  auto FuncInsert=[&](double const& date, std::vector<int> const& eTimePos, RadarMeasLine const& eMeas) -> void {
    bool IsCorrect;
    if (pos == -1) {
      IsCorrect=false;
    }
  ...
  while(!INfs.eof()) {
	...
	//	std::cerr << "Step 6\n";
	//	std::cerr << "lon/lat=" << lon << "/" << lat << " Wheight=" << Wheight << " Wdir=" << Wdir << "\n";
...
DATAaqua ReadAquaSatellite(std::string const& DataFile, std::string const& VarName)
{
  std::cerr << "Beginning of ReadAquaSatellite DataFile=" << DataFile << "\n";
  if (!IsExistingFile(DataFile)) {
    std::cerr << "ReadAquaSatellite : The file DataFile = " << DataFile << " is missing\n";
    throw TerminalException{1};
  }
  ...
  std::cerr << "pixels_per_line=" << pixels_per_line << "   number_of_lines=" << number_of_lines << "\n";
  //
  // Reading groups
  //
  //  std::cerr << "Reading groups\n";
  netCDF::NcGroup GrpGeophys=dataFile.getGroup("geophysical_data", netCDF::NcGroup::ChildrenGrps);
  ...
  //
  netCDF::NcGroup GrpLine=dataFile.getGroup("scan_line_attributes", netCDF::NcGroup::ChildrenGrps);
  if (GrpLine.isNull()) {
    std::cerr << "ReadAquaSatellite : error when accessing scan_line_attributes\n";
    std::cerr << "DataFile = " << DataFile << "\n";
    throw TerminalException{1};
  }
  //  std::cerr << "We have GrpLine\n";
  //
  netCDF::NcGroup GrpNav=dataFile.getGroup("navigation_data", netCDF::NcGroup::ChildrenGrps);
  if (GrpLine.isNull()) {
  ...
  if (DoPrintGroupName) {
    std::multimap<std::string, netCDF::NcGroup> ListGroup=dataFile.getGroups(netCDF::NcGroup::ChildrenGrps);
  ...
  for (int iTime=0; iTime<nbTime; iTime++) {
    ...
    /*    if (iTime == 0 || iTime == nbTime-1) {
      ...
      std::cerr << "  eTimeDay1=" << eTimeDay1 << "\n";
      std::cerr << "  eTimeFinal=" << eTimeFinal << "\n";
      }*/
  ...
  //
  /*
  std::vector<std::string> ListVarName{"chlor_a", "chl_ocx", "pic", "poc", "ipar", "par"};
  for (auto & eVar : ListVarName) {
    netCDF::NcVar data_fill=GrpGeophys.getVar(eVar);
    int nbFill=NC_ReadVariable_NbFillValue_data(data_fill);
    std::cerr << "eVar=" << eVar << " nbFill=" << nbFill << "\n";
    }*/
  //
  // Reading data
  //
  ...
  if (nbRow != number_of_lines || nbCol != pixels_per_line) {
    ...
    std::cerr << "But pixels_per_line=" << pixels_per_line << " number_of_lines=" << number_of_lines << "\n";
    throw TerminalException{1};
  }
  //
...
std::vector<std::string> GetAquaListLinkDownload_Specified(FullNamelist const& eFull, double const& MjdFirst, double const& MjdLast)
{
  ...
  auto GetSpecificDay=[&](double const& eDay) -> void {
    ...
    std::string TmpStdout=TheDir + "tmp_" + StringNumber(eYear,4) + "_" + strDay + "_stdout";
    std::string TmpStderr=TheDir + "tmp_" + StringNumber(eYear,4) + "_" + strDay + "_stderr";
    std::string eComm1="(cd " + TheDir + " && wget " + AquaLink + StringNumber(eYear,4) + "/" + strDay + " > " + TmpStdout + " 2> " + TmpStderr + ")";
    int iret=system(eComm1.c_str() );
    std::cerr << "iret=" << iret << "\n";
    if (iret == 0) {
      std::cerr << "File correctly downloaded |ListStringLink|=" << ListStringLink.size() << "\n";
  ...
  std::cerr << "Now leaving GetAquaListLinkDownload_Specified, |ListStringLink|=" << ListStringLink.size() << "\n";
