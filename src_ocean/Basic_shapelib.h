...
ShapefileData ExtractShapefileFromUnstructuredMesh(GridArray const& GrdArr, std::vector<std::vector<int>> const& eListList)
{
  ...
  for (auto & eList : eListList) {
    ...
    int nSHPType=3;
    int nShapeId=iIns;
    ListBlock.push_back({eVectQuad, iIns, iInsPart, ePartType, nSHPType, nShapeId});
    iIns++;
  }
  //
  ShapefileData eSHP;
...
void PrintShapefileData(std::ostream & os, ShapefileData const& eShape)
{
  ...
  for (int i=0; i<4; i++) {
    os << "i=" << i << " minBound=" << eShape.minbound[i] << " maxbound=" << eShape.maxbound[i] << "\n";
  }
  ...
  for (int iBlock=0; iBlock<nbBlock; iBlock++) {
    ...
    os << "  ePartType=" << ePartType << " nSHPType=" << nSHPType << " nShapeId=" << nShapeId << "\n";
    ...
    double ErrCycle=fabs(ListX[0] - ListX[len-1]) + fabs(ListY[0] - ListY[len-1]);
...
int SHP_GetShapeType(std::string const& eFile)
{
  ...
  for (int i=0; i<4; i++) {
    std::cerr << "i=" << i << " minbound=" << minbound[i] << " maxbound=" << maxbound[i] << "\n";
  }
...
ShapefileData SHP_ReadFromNetcdf(std::string const& eFile)
{
  ...
  for (int iPart=0; iPart<nParts; iPart++) {
    ...
    ListBlock.push_back({eVectQuad, iIns, iPartIns, ePartType, nSHPType, nShapeId});
  }
  std::vector<double> ListX(nVertices), ListY(nVertices), ListZ(nVertices), ListM(nVertices);
  for (int i=0; i<nVertices; i++) {
  ...
  std::vector<double> vectMinBound{VectorMin(ListX), VectorMin(ListY), VectorMin(ListZ), VectorMin(ListM)};
  std::vector<double> vectMaxBound{VectorMax(ListX), VectorMax(ListY), VectorMax(ListZ), VectorMax(ListM)};
  int tshp=3;
  int nshp=nParts;
  ShapefileData eSHP;
  eSHP.eFile=eFile;
...
void SHP_WriteGmshInputFile(std::string const& eFile, ShapefileData const& eSHP)
{
...
void SHP_WriteNetcdfFile(std::string const& eFile, ShapefileData const& eSHP)
{
  ...
  netCDF::NcVar dataPanPartStart  =dataFile.addVar("panPartStart", "int", ListDim1);
  netCDF::NcVar dataPanPartType   =dataFile.addVar("panPartType", "int", ListDim1);
  netCDF::NcVar dataList_nSHPType =dataFile.addVar("List_nSHPType", "int", ListDim1);
  netCDF::NcVar dataList_nShapeId =dataFile.addVar("List_nShapeId", "int", ListDim1);
  //
...
ShapefileData SHP_ReadShapefile(std::string const& eFile)
{
  ...
  if (tshp == SHPT_NULL || tshp == SHPT_POINT || tshp == SHPT_MULTIPOINT || tshp == SHPT_POINTZ || 
      tshp == SHPT_MULTIPOINTZ || tshp == SHPT_POINTM || tshp == SHPT_MULTIPOINTM || tshp == SHPT_MULTIPATCH) {
...
void DBF_PrintDatabaseInfo(std::ostream & os, std::string const& eFile)
{
  ...
  for (int iField=0; iField<nField; iField++) {
    ...
    DBFFieldType eType=DBFGetFieldInfo(dbfhandle, iField, FieldName, &pnWidth, &pnDecimals);
    std::string type;
    if (eType == FTString)
      type="string";
    if (eType == FTInteger)
    ...
    os << "iField=" << iField << " name=" << strFieldName << " type=" << type << " pnWidth=" << pnWidth << " pnDecimals=" << pnDecimals << "\n";
  ...
  for (int iRecord=0; iRecord<nRecord; iRecord++) {
	...
	str=ptr;
      ...
      os << "  iField=" << iField << " type=" << type << " name=" << ListName[iField] << " val=" << str << "\n";
    }
  }
  DBFClose(dbfhandle);
}
...
DBFdataset DBF_ReadDataset(std::string const& eFile)
{
  ...
  std::vector<std::string> ListIntVar, ListDoubleVar, ListBoolVar, ListStringVar;
  ...
  for (int iField=0; iField<nField; iField++) {
    ...
    if (eType == FTInvalid)
      type="invalid";
    std::cerr << "iField=" << iField << " name=" << strFieldName << " type=" << type << " pnWidth=" << pnWidth << " pnDecimals=" << pnDecimals << "\n";
    ListType.push_back(type);
  }
  std::vector<int> ListIntValue;
  std::vector<double> ListDoubleValue;
  ...
  for (int iRecord=0; iRecord<nRecord; iRecord++) {
	...
	std::cerr << "In dearth of an example, that is unfortunately quite hard\n";
...
void DBF_WriteDataset(std::string const& eFile, DBFdataset const& dds)
{
  ...
  for (auto & eVar : dds.ListBoolVar)
    DBFAddField(dbfhandle, eVar.c_str(), FTLogical, 11, 0); // We do not really know actually
  //
  ...
  for (int iRecord=0; iRecord<nRecord; iRecord++) {
    ...
    for (int i=0; i<nbIntVar; i++) {
      int ins=DBFWriteIntegerAttribute(dbfhandle, iRecord, pos, dds.ListIntValue[idxInt]);
      if (ins == 0) {
    ...
    for (int i=0; i<nbDoubleVar; i++) {
      int ins=DBFWriteDoubleAttribute(dbfhandle, iRecord, pos, dds.ListDoubleValue[idxDouble]);
      if (ins == 0) {
    ...
    for (int i=0; i<nbStringVar; i++) {
      int ins=DBFWriteStringAttribute(dbfhandle, iRecord, pos, dds.ListStringValue[idxDouble].c_str());
      if (ins == 0) {
...
void WriteCoastlineInformation(std::string const& eFile, ShapefileData const& eSHP)
{
  std::string eExtension=FILE_GetExtension(eFile);
  if (eExtension == "shp") {
