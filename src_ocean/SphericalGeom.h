...
bool IsPointInQuadArray(QuadArray const& eQuad, double const& eLon, double const& eLat)
{
...
std::string ConvertLLDoubleToDegMinSec(double const& eLL)
{
  ...
  std::string retStr=IntToString(eDeg) + "." + IntToString(eMin) + "." + IntToString(eSec);
  return retStr;
}
...
std::vector<PairLL> GetGridBoundaryTot(MyMatrix<double> const& LON, MyMatrix<double> const& LAT)
{
  int nbRow=LON.rows();
  int nbCol=LON.cols();
...
std::vector<double> GetXYZcoordinateLL(double const& LonDeg, double const& LatDeg)
{
  double pi=3.141592653589792;
...
double SphericalCoordinateArea(double const& eLon1, double const& eLon2, double const& eLon3, double const& eLat1, double const& eLat2, double const& eLat3)
{
...
double SphericalCoordinateAreaKM(double const& eLon1, double const& eLon2, double const& eLon3, double const& eLat1, double const& eLat2, double const& eLat3)
{
  double EarthRadius=6370;
  double area=SphericalCoordinateArea(eLon1, eLon2, eLon3, eLat1, eLat2, eLat3);
  return EarthRadius*EarthRadius*area;
}


template<typename T>
bool get_line_intersection(T const& p0_x, T const& p0_y, T const& p1_x, T const& p1_y,
			   T const& p2_x, T const& p2_y, T const& p3_x, T const& p3_y,
			   T *i_x, T *i_y)
{
  T s1_x = p1_x - p0_x;
  T s1_y = p1_y - p0_y;
  ...
  T s = (-s1_y * (p0_x - p2_x) + s1_x * (p0_y - p2_y)) / (-s2_x * s1_y + s1_x * s2_y);
  T t = ( s2_x * (p0_y - p2_y) - s2_y * (p0_x - p2_x)) / (-s2_x * s1_y + s1_x * s2_y);
...
// The question is whether LonDomain2 / LatDomain2 intersects with Domain1 or is contained in it.
bool IsContainedInDomain(std::vector<double> const& LonDomain1, std::vector<double> const& LatDomain1, std::vector<double> const& LonDomain2, std::vector<double> const& LatDomain2)
{
  ...
  //
  auto fastConclusionExclusion=[](std::vector<double> const& Field1, std::vector<double> const& Field2) -> bool {
  ...
  for (int i1=0; i1<siz1; i1++) {
    ...
    for (int i2=0; i2<siz2; i2++) {
      ...
      double int_x, int_y;
      bool test=get_line_intersection(p0_x, p0_y, p1_x, p1_y,
				      p2_x, p2_y, p3_x, p3_y,
				      &int_x, &int_y);
      if (test)
	return true;
...
MyMatrix<double> GRID_GetMaxRadiusInfluence_kernel(MyMatrix<double> const& LON_rho, MyMatrix<double> const& LAT_rho, std::string const& strDistType)
{
