...
std::vector<VarQuery> GetIntervalFLD_query(std::vector<double> const& ListTime, double const& TimeFrameDay)
{
...
std::vector<VarQuery> GetIntervalGen_Kernel(SingleBlock const& eBlock, double const& PropFirstTime, double const& PropLastTime, double const& PropDeltaInterval)
{
  ...
  if (BEGTC == "earliest") {
    if (PropFirstTime < 0) {
      std::cerr << "PropFirstTime has not been set. This should mean that no model has been put on input\n";
      std::cerr << "Combined with the choice of \"earliest\" for first time this make the system impossible to determine the time\n";
      throw TerminalException{1};
    }
  ...
  if (ENDTC == "latest") {
    if (PropLastTime < 0) {
      std::cerr << "PropLastTime has not been set. This should mean that no model has been put on input\n";
      std::cerr << "Combined with the choice of \"earliest\" for first time this make the system impossible to determine the time\n";
      throw TerminalException{1};
    }
  ...
  if (KindSelect == "specific") {
    std::vector<std::string> ListStringTime=eBlock.ListListStringValues.at("ListSpecificTimes");
...
std::vector<VarQuery> GetIntervalGen_Query(SingleBlock const& eBlock, std::vector<ArrayHistory> const& ListArr)
{
...
MyMatrix<double> NETCDF_Get2DvariableSpecTime(TotalArrGetData const& TotalArr, std::string const& eVar, double const& eTimeDay)
{
  ...
  MyMatrix<double> eVarLow=NETCDF_Get2DvariableSpecEntry(HisFileLow, TotalArr.GrdArr, eVar, iRecLow);
  MyMatrix<double> eVarUpp=NETCDF_Get2DvariableSpecEntry(HisFileUpp, TotalArr.GrdArr, eVar, iRecUpp);
...
Eigen::Tensor<double,3> NETCDF_Get3DvariableSpecTime(TotalArrGetData const& TotalArr, std::string const& eVar, double const& eTimeDay)
{
  ...
  std::string HisFileLow=ARR_GetHisFileName(TotalArr.eArr, eVar, iFileLow);
  std::string HisFileUpp=ARR_GetHisFileName(TotalArr.eArr, eVar, iFileUpp);
  Eigen::Tensor<double,3> eVarLow=NETCDF_Get3DvariableSpecEntry(HisFileLow, TotalArr.GrdArr, eVar, iRecLow);
  Eigen::Tensor<double,3> eVarUpp=NETCDF_Get3DvariableSpecEntry(HisFileUpp, TotalArr.GrdArr, eVar, iRecUpp);
