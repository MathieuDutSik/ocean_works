...
int main(int argc, char *argv[])
{
  ...
  try {
    ...
    std::cerr << "LON(min/max/avg)=" << minLon << " , " << maxLon << " , " << avgLon << "\n";
    std::cerr << "LAT(min/max/avg)=" << minLat << " , " << maxLat << " , " << avgLat << "\n";
    //
    std::vector<double> ListTime=NC_ReadTimeFromFile(eFileName, "ocean_time");
    int nbTime=ListTime.size();
    //
    for (int iTime=0; iTime<nbTime; iTime++) {
      MyMatrix<double> UsurfCurr=NETCDF_Get2DvariableSpecEntry(eFileName, GrdArr, "UsurfCurr", iTime);
      MyMatrix<double> VsurfCurr=NETCDF_Get2DvariableSpecEntry(eFileName, GrdArr, "VsurfCurr", iTime);
      MyMatrix<double> Hwave=NETCDF_Get2DvariableSpecEntry(eFileName, GrdArr, "HS", iTime);
      MyMatrix<double> TM02=NETCDF_Get2DvariableSpecEntry(eFileName, GrdArr, "TM02", iTime);
      MyMatrix<double> MAPSTA=NETCDF_Get2DvariableSpecEntry(eFileName, GrdArr, "MAPSTA", iTime);
