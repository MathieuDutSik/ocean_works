...
std::vector<SingleRecInterp> TRIG_FIND_ELE(MyMatrix<int> const& INE, MyMatrix<double> const& X, MyMatrix<double> const& Y, QuadCoordinate const& eQuad, MyMatrix<double> const& ListXY, bool const& AllowExtrapolation)
{
  ...
  std::pair<std::vector<int>,std::vector<int>> ePairSort = SortingLists(ListElt);
...
std::vector<SingleRecInterp> NearestInterpolation_FindWeight_LonLat(MyMatrix<double> const& LON, MyMatrix<double> const& LAT, MyMatrix<double> const& ListXY)
{
  ...
  for (int iPoint=0; iPoint<nbPoint; iPoint++) {
    ...
    flann_index.knnSearch(query, indices, dists, knn, cv::flann::SearchParams(32));
    if (indices.rows != 1) {
...
std::vector<SingleRecInterp> NearestInterpolation_FindWeight_FD(GridArray const& GrdArr, MyMatrix<double> const& ListXY)
{
  ...
  std::vector<SingleRecInterp> LRec = NearestInterpolation_FindWeight_LonLat(LON, LAT, ListXY);
