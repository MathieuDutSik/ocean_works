...
double
ComputeHydrostaticInconsistencyNumber(Eigen::Tensor<double, 3> const &z_w,
                                      GridArray const &GrdArr) {
  auto compute_rx1 = [&](int const &iEta, int const &iXi, int const &jEta,
                         int const &jXi, int k) -> double {
          ...
          if (iEtaN >= 0 && iEtaN < eta_rho && iXiN >= 0 && iXiN < xi_rho) {
            if (GrdArr.GrdArrRho.MSK(iEtaN, iXiN)) {
              for (int k = 1; k <= N; k++) {
                ...
                if (val > rx1) {
                  //                  std::cerr << "Now rx1=" << val << "
                  //                  iEta/iXi=" << iEta << "/" << iXi << "
                  //                  iEtaN/iXiN=" << iEtaN << "/" << iXiN <<
                  //                  "\n"; std::cerr << "   dep1=" <<
                  //                  GrdArr.GrdArrRho.DEP(iEta,iXi) << " dep2="
                  //                  << GrdArr.GrdArrRho.DEP(iEtaN,iXiN) << "
                  //                  k=" << k << "\n";
                  rx1 = val;
                }
              }
...
void DiagnosticsVerticalStratificationDiagnostic(FullNamelist const &eFull) {
  ...
  ARVDtyp ARVD = ROMSgetARrayVerticalDescription(N, Vtransform, Vstretching,
                                                 Tcline, hc, ThetaS, ThetaB);
  GrdArr.ARVD = ARVD;
  for (int i = 0; i <= ARVD.N; i++)
    std::cerr << "i=" << i << " sc_w=" << ARVD.sc_w(i)
              << " Cs_w=" << ARVD.Cs_w(i) << "\n";

  int eta_rho = GrdArr.GrdArrRho.LON.rows();
  int xi_rho = GrdArr.GrdArrRho.LON.cols();
  MyMatrix<double> zeta = ZeroMatrix<double>(eta_rho, xi_rho);
  Eigen::Tensor<double, 3> z_w =
      ROMS_ComputeVerticalGlobalCoordinate_w(GrdArr, zeta);
  Eigen::Tensor<double, 3> z_r =
  ...
  std::cerr << "minDep=" << minDep << " maxDep=" << maxDep
            << " avgDep=" << avgDep << "\n";
}

MyMatrix<double> My_u2rho(MyMatrix<double> const &eVar_u,
                          MyMatrix<uint8_t> const &MSK_u) {
  ...
  if (!IsEqualSizeMatrices(eVar_u, MSK_u)) {
    ...
    std::cerr
        << "Most likely the grid array does not match the chosen history\n";
...
MyMatrix<double> My_v2rho(MyMatrix<double> const &eVar_v,
                          MyMatrix<uint8_t> const &MSK_v) {
...
Eigen::Tensor<double, 3> My_v2rho_3D(Eigen::Tensor<double, 3> const &eVar_v,
                                     MyMatrix<uint8_t> const &MSK_v) {
...
MyMatrix<double> My_rho2u_2D(GridArray const &GrdArr,
...
MyMatrix<double> My_rho2v_2D(GridArray const &GrdArr,
                             MyMatrix<double> const &M) {
  int eta_rho = M.rows();
  int xi_rho = M.cols();
  int eta_v = eta_rho - 1;
...
Eigen::Tensor<double, 3> My_rho2u_3D(GridArray const &GrdArr,
                                     Eigen::Tensor<double, 3> const &M) {
...
Eigen::Tensor<double, 3> My_rho2v_3D(GridArray const &GrdArr,
                                     Eigen::Tensor<double, 3> const &M) {
  auto LDim = M.dimensions();
  int s_vert = LDim[0];
  int eta_rho = LDim[1];
...
Eigen::Tensor<double, 3> My_u2rho_3D(Eigen::Tensor<double, 3> const &eVar_u,
                                     MyMatrix<uint8_t> const &MSK_u) {
...
void AngleRhoRot(MyMatrix<double> &U_rho, MyMatrix<double> &V_rho,
  ...
  if (U_rho.rows() != ANG.rows() || V_rho.rows() != ANG.rows() ||
      U_rho.cols() != ANG.cols() || V_rho.cols() != ANG.cols()) {
    ...
    std::cerr
        << "Most likely the grid used does not match the history file used\n";
    throw TerminalException{1};
  }
...
void AngleRhoRot_3D(Eigen::Tensor<double, 3> &U_rho,
                    Eigen::Tensor<double, 3> &V_rho,
                    MyMatrix<double> const &ANG) {
...
std::vector<std::string> GetListVariableBFM_full() {
  std::vector<std::string> ListVar{
      "iOxyg", "iPO4_", "iNO3_", "iNH4_", "iO4n_", "iSiOH", "iN6r_", "iB1c_",
      "iB1n_", "iB1p_", "iP1c_", "iP1n_", "iP1p_", "iP1l_", "iP1s_", "iP2c_",
      "iP2n_", "iP2p_", "iP2l_", "iP3c_", "iP3n_", "iP3p_", "iP3l_", "iP4c_",
      "iP4n_", "iP4p_", "iP4l_", "iZ3c_", "iZ3n_", "iZ3p_", "iZ4c_", "iZ4n_",
      "iZ4p_", "iZ5c_", "iZ5n_", "iZ5p_", "iZ6c_", "iZ6n_", "iZ6p_", "iR1c_",
      "iR1n_", "iR1p_", "iR2c_", "iR3c_", "iR6c_", "iR6n_", "iR6p_", "iR6s_",
      "iO3c_", "iO3h_", "iEIR_", "iDIC_", "iChlo", "siP1_", "siP2_", "siP3_",
      "siP4_", "eiP1_", "eiP2_", "eiP3_", "eiP4_", "ruPTc", "ruZTc", "ixEPS"};
  return ListVar;
}
...
std::vector<VarRomsDesc>
GetFullVariablesNames(std::vector<std::string> const &ListVar,
                      std::string const &VarInfoFile) {
  ...
  for (int iVar = 0; iVar < nbVar; iVar++) {
    ...
    std::cerr << "iVar=" << iVar << " / " << nbVar
              << " eStrSearch=" << eStrSearch << "\n";
    int iLineFound = -1;
    for (int iLine = 0; iLine < nbLine; iLine++) {
  ...
  auto GetLineKeyString = [&](int const &iLine) -> std::string {
    if (LineHasString[iLine] == 0) {
      std::cerr
          << "The Line has no ' in it, therefore extraction is impossible\n";
      throw TerminalException{1};
    }
  ...
  for (int iVar = 0; iVar < nbVar; iVar++) {
    ...
    double ScalarMult;
    is >> ScalarMult;
    ListFullVarRomsDesc[iVar] = {NetcdfName, FullName, Unit,
                                 ScalarMult, KeyStr,   shortStr};
  }
...
Eigen::Tensor<double, 3> GetConditionsAccordingToDescription(
    GridArray const &GrdArr, ARVDtyp const &ARVD, FullNamelist const &eFullDesc,
    VarRomsDesc const &eVarRomsDesc) {
  int N = ARVD.N;
  int eta_rho = GrdArr.GrdArrRho.LON.rows();
  int xi_rho = GrdArr.GrdArrRho.LON.cols();
  ...
  if (MethodSetting == "VerticalProfile") {
    ...
    std::vector<double> retListVal =
        OneDimInterpolation_vector(ListVal_V, ListDep_V, VectZ);
    idx = 0;
    for (int i = 0; i < eta_rho; i++)
      for (int j = 0; j < xi_rho; j++) {
        for (int k = 0; k < N; k++) {
...
MyMatrix<double> My_Psi2Rho(MyMatrix<double> const &F_psi) {
  ...
  F_rho(eta_rho - 1, xi_rho - 1) =
      (F_rho(eta_rho - 1, xi_rho - 2) + F_rho(eta_rho - 2, xi_rho - 1)) / 2;
  return F_rho;
}
...
void print_matrix_info(std::string name, MyMatrix<double> const &M) {
  ...
  std::cerr << "name=" << name << " |M|=" << M.rows() << " / " << M.cols()
            << " min_c=" << min_c << " max_c=" << max_c << "\n";
}

MyMatrix<double> ZeroMask(MyMatrix<double> const &M,
...
MyMatrix<double> GRID_VorticityRho(const GridArray &GrdArr,
                                   MyMatrix<double> const &U,
...
void SetNetcdfInitial(FullNamelist const &eFull) {
  ...
  std::vector<VarRomsDesc> ListVarRomsDesc =
      GetFullVariablesNames(ListVar, VarInfoFile);
  ...
  if (!IsExistingFile(NetcdfInitialFile)) {
    std::cerr << "Error the file NetcdfInitialFile=" << NetcdfInitialFile
              << " is missing\n";
    std::cerr << "Please correct\n";
    throw TerminalException{1};
  }
  netCDF::NcFile dataFile(NetcdfInitialFile, netCDF::NcFile::write);
  std::vector<std::string> ListDimField{"ocean_time", "s_rho", "eta_rho",
                                        "xi_rho"};
  ...
  for (auto &eVarRomsDesc : ListVarRomsDesc) {
    ...
    std::cerr << "Treating variable shortStr=" << shortStr
              << " eDescFile=" << eDescFile
              << " NetcdfName=" << eVarRomsDesc.NetcdfName << "\n";
    if (IsExistingFile(eDescFile)) {
      ...
      Eigen::Tensor<double, 3> eTensTracer =
          GetConditionsAccordingToDescription(GrdArr, eARVD, eFullDesc,
                                              eVarRomsDesc);
      std::cerr << "eTensTracer read\n";
      std::vector<double> Vstat = GetMinMaxAvg(eTensTracer);
      std::cerr << "min=" << Vstat[0] << " max=" << Vstat[1]
                << " avg=" << Vstat[2] << "\n";
      if (!NC_IsVar(NetcdfInitialFile, eVarRomsDesc.NetcdfName)) {
        netCDF::NcVar eVarData =
            dataFile.addVar(eVarRomsDesc.NetcdfName, "float", ListDimField);
...
void CopyTracerFields(FullNamelist const &eFull) {
  ...
  for (auto &eVarRomsDesc : ListVarRomsDesc) {
    std::string FullFile =
        PrefixVariableDefinitions + eVarRomsDesc.shortStr + ".nml";
    //
    std::string eVarNetcdf = eVarRomsDesc.NetcdfName;
    std::cerr << "Treating eVarNetcdf = " << eVarNetcdf << "\n";
    //
    Eigen::Tensor<double, 3> TheTens = NETCDF_Get3DvariableSpecEntry_FD(
        NetcdfFileIn, GrdArr, eVarNetcdf, NetcdfIdxIn);
    NETCDF_Write3DvariableSpecEntry(NetcdfFileOut, eVarNetcdf, NetcdfIdxOut,
                                    TheTens);
  }
}
