...
std::vector<PointOutTrans> ReadStationCoordinate(SingleBlock const& eBlPLOT)
{
  ...
  if (type == "namelist") {
    std::vector<double> ListPointLon=eBlPLOT.ListListDoubleValues.at("ListPointLon");
    std::vector<double> ListPointLat=eBlPLOT.ListListDoubleValues.at("ListPointLat");
    std::vector<double> ListPointDep=eBlPLOT.ListListDoubleValues.at("ListPointDepth");
    std::vector<std::string> ListPointName=eBlPLOT.ListListStringValues.at("ListPointName");
...
void TRANSECT_Plot(FullNamelist const& eFull)
{
  ...
  if (nbPointOut > 0) {
    ...
    for (int iGrid=0; iGrid<nbGrid; iGrid++)
      ListTrans3D[iGrid]=RetrievePointTransectRecord(ListTotalArr[iGrid], ListXY, VertResolM);
    for (int iPt=0; iPt<nbPointOut; iPt++) {
  ...
  //
  std::vector<TransectInformation> ListTransect=RetrieveListTransect(eBlPLOT, ListGrdArr);
  ...
  std::vector<VarQuery> ListQuery=GetIntervalGen_Query(eBlPROC, ListArrayHistory);
  ...
  for (int iTime=0; iTime<nbTime; iTime++) {
	...
	for (int iGrid=0; iGrid<nbGrid; iGrid++)
	  ListRecVar[iGrid]=ModelSpecificVarSpecificTimeGeneral(ListTotalArr[iGrid], eVarName, eQuery, ePlotBound);
	for (int iTrans=0; iTrans<nbTrans; iTrans++) {
	  ...
	  for (int iGrid=0; iGrid<nbGrid; iGrid++) {
	    RecVar eRecVar=INTERPOL_SingleRecVarInterpolation(ListTransect[iTrans].ListRec[iGrid], ListRecVar[iGrid]);
	    MyVector<double> eVect(nbPoint);
	    for (int iPoint=0; iPoint<nbPoint; iPoint++) {
	      eVect(iPoint)=eRecVar.F(iPoint,0);
	      // std::cerr << "iPoint=" << iPoint << " val=" << eRecVar.F(iPoint,0) << "\n";
	    }
	...
	if (nbPointOut > 0) {
	  ...
	  for (int iGrid=0; iGrid<nbGrid; iGrid++)
	    ListData[iGrid]=TransectInterpolation_3D(ListTrans3D[iGrid], ListRecVar[iGrid].Tens3);
	  for (int iPt=0; iPt<nbPointOut; iPt++) {
	    ...
	    std::cerr << "iTime=" << iTime << "/" << nbTime << " iPt=" << iPt << "/" << nbPointOut << " name=" << ListPointOut[iPt].name << " lon=" << ListPointOut[iPt].lon << " lat=" << ListPointOut[iPt].lat << "\n";
	    ...
	    for (int iGrid=0; iGrid<nbGrid; iGrid++) {
	      ...
	      std::cerr << "ListData[iGrid](rows/cols)=" << ListData[iGrid].rows() << " / " << ListData[iGrid].cols() << "\n";
	      for (int i=0; i<nbVert; i++) {
		int j=nbData - 1 - i;
		double eVal=ListData[iGrid](j, iPt);
		int eMSK=ListTrans3D[iGrid].MSK(j, iPt);
		std::cerr << "    i=" << i << " eVal=" << eVal << " eMSK=" << eMSK << "\n";
		eVect(i) = eVal;
	      }
	    ...
	    DrawLinesArr eDrawArr;
	    eDrawArr.DoTitle=true;
	    eDrawArr.TitleStr="Trans. " + name + " of " + eVarName + " at " + strPres;
	    std::string fVarName="Vertical_transect_" + name + "_" + eVarName + "_" + strFile;
	    eDrawArr.IsTimeSeries=false;
	    eDrawArr.PairComparison=false;
	    eDrawArr.DoExplicitLabel=false;
            eDrawArr.DrawHorizVertLines=false; // May be put as input parameter.
	    ...
	    if (nbVert > 0 && TheMax < 10000) {
	      eDrawArr.ListListVect=ListListVect;
	      std::string FileName=ePerm.eDir + "VertTransect_" + name + "_" + eVarName + "_" + StringNumber(iTime,4) + "_at_" + strFile;
	      LINES_PLOT(FileName, eDrawArr, eCall, ePerm);
	    }
	  }
	}
