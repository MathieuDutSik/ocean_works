...
int main(int argc, char *argv[]) {
  ...
  try {
    if (argc != 12) {
      std::cerr << "GRID_CompWWMboundary [GridFile] [NorthStat] [WestStat] "
                   "[SouthStat] [EastStat] [NorthVal] [WestVal] [SouthVal] "
                   "[EastVal] [tolLL] [WWMboundary]\n";
      std::cerr << "\n";
      std::cerr << "GridFile a WWM grid file (type determined by suffix\n";
      std::cerr << "NorthStat, WestStat, SouthStat and EastStat specifies "
                   "boundary status\n";
    ...
    std::cerr << "NorthStat = " << NorthStat << " argv=" << argv[2]
              << " NorthVal=" << NorthVal << "\n";
    std::cerr << "WestStat  = " << WestStat << " argv=" << argv[3]
              << "  WestVal=" << WestVal << "\n";
    std::cerr << "SouthStat = " << SouthStat << " argv=" << argv[4]
              << " SouthVal=" << SouthVal << "\n";
    std::cerr << "EastStat  = " << EastStat << " argv=" << argv[5]
              << "  EastVal=" << EastVal << "\n";
    //
    double tolLL;
    ...
    std::cerr << "nbSouth=" << nbSouth << " nbNorth=" << nbNorth
              << " nbEast=" << nbEast << " nbWest=" << nbWest << "\n";
