...
std::vector<PointOutTrans>
ReadStationCoordinate_File(std::string const &eFile) {
...
std::vector<bool> DetermineBelonging_ListXY(GridArray const &GrdArr,
                                            MyMatrix<double> const &ListXY) {
  int nbPoint = ListXY.cols();
  std::vector<SingleRecInterp> LSingle =
      General_FindInterpolationWeight(GrdArr, ListXY, false);
...
SingleArrayRegionAveraging ComputeArrayRegionAveraging_ListPolygon(
    GridArray const &GrdArr,
    std::vector<std::pair<std::vector<double>, std::vector<double>>> const
        &ListRegions) {
...
ComputeArrayInterpolation_ListXY(GridArray const &GrdArr,
                                 MyMatrix<double> const &ListXY) {
  int nbPoint = ListXY.cols();
  std::vector<SingleRecInterp> LSingle =
      General_FindInterpolationWeight(GrdArr, ListXY, false);
  ...
  for (int i = 0; i < nbPoint; i++) {
    ...
    //    std::cerr << "ComputeArrayInterpolation i=" << i << " status=" <<
    //    LSingle[i].status << "\n";
  }
  if (nbError > 0) {
...
TransectInformation
GetTransectInformation(std::vector<GridArray> const &ListGrdArr,
  ...
  for (int iGrid = 0; iGrid < nbGrid; iGrid++) {
    // Possibly we need better construction of the Generalized interpolation
    // arrays
    eTransect.ListRec.push_back(
        {ComputeArrayInterpolation_ListXY(ListGrdArr[iGrid], ListXY), {}});
  }
  return eTransect;
}

TransectInformation_3D GetTransectInformation_3D(
    TransectInformation const &eTrans, GridArray const &GrdArr,
    Eigen::Tensor<double, 3> const VertCoord, double const &VertResolM) {
  ...
  int Nvert = LDim[0];
  Eigen::Tensor<double, 3> TheRes =
      SingleInterpolationOfField_3D(eTrans.ListRec[0], VertCoord);
  ...
  //
  double deltaLON =
      eTrans.ListPairLL[nbPoint - 1].eLon - eTrans.ListPairLL[0].eLon;
  double deltaLAT =
      eTrans.ListPairLL[nbPoint - 1].eLat - eTrans.ListPairLL[0].eLat;
...
MyMatrix<double> TransectInterpolation_3D(TransectInformation_3D const &eTrans3,
                                          Eigen::Tensor<double, 3> const &F) {
  //  std::cerr << "max(F)=" << maxCoeff(F) << "\n";
  Eigen::Tensor<double, 3> F2 =
      SingleInterpolationOfField_3D(eTrans3.eRecInterp, F);
...
std::vector<TransectInformation>
RetrieveListTransect(SingleBlock const &eBlPLOT,
  ...
  if (nbTrans_t != ListLonStart.size() || nbTrans_t != ListLatStart.size() ||
    ...
    std::cerr << "TransectLonStart, TransectLatStart, TransectLonEnd, "
                 "TransectLatEnd and SpatialResolutionTransectKM\n";
    throw TerminalException{1};
  }
  std::vector<TransectInformation> ListTransect(nbTrans);
  for (int iTrans = 0; iTrans < nbTrans; iTrans++)
    ListTransect[iTrans] = GetTransectInformation(
        ListGrdArr, ListLonStart[iTrans], ListLatStart[iTrans],
        ListLonEnd[iTrans], ListLatEnd[iTrans], ListResolKM[iTrans]);
  return ListTransect;
}
