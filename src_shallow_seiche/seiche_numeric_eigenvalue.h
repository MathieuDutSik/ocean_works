...
std::string GetPairVectorInfo(MyVector<double> const &V1,
                              MyVector<double> const &V2) {
  ...
  return "norm1=" + std::to_string(scal11) +
         " norm2=" + std::to_string(scal22) +
         " scal_N=" + std::to_string(scal_N) + " quot=" + std::to_string(quot);
}
...
/*
  ...
  ---
  Here h0 is the surface level. b is the height and h0 - b is the total water
  height. Thus we set b = -h with h the depth.
  ---
  An eigenvalue lambda of the quadratic form will give us via
  lambda = omega^2 / g
 */
std::vector<PeriodicSolution>
ComputeEigenvaluesSWE1(double const &h0, int const &maxNbEig,
                       GridArray const &GrdArr, bool const &RescaleEigenvector,
                       bool const &UseSI_Bmatrix, int const &ncv) {
  ...
  std::pair<std::vector<size_t>, std::vector<size_t>> ePair =
      eG.Get_ListStart_ListListAdj();
  ...
  for (int i_elt = 0; i_elt < nb_elt; i_elt++) {
    ...
    /*  The linear function is f(x,y) = a + bx + cy over the triangle.
        ...
        b [(x1 - x0) (y2 - y0) - (x2 - x0) (y1 - y0)] = (y2 - y0) (f1 - f0) -
       (y1 - y0) (f2 - f0) c [(x1 - x0) (y2 - y0) - (x2 - x0) (y1 - y0)] = (x1 -
       x0) (f2 - f0) - (x2 - x0) (f1 - f0) Define Delta = (x1 - x0) (y2 - y0) -
       (x2 - x0) (y1 - y0) We thus have: b Delta = f0 (y1 - y2) + f1 (y2 - y0) +
       f2 (y0 - y1) = b0 f0 + b1 f1 + b2 f2 c Delta = f0 (x2 - x1) + f1 (x0 -
       x2) + f2 (x1 - x0) = c0 f0 + c1 f1 + c2 f2 The gradient term to put is
       thus: avg(h - B) * Area(Delta) * [b*b + c*c] The integral term \int_T f^2
       is thus P = P0 + u (P1 - P0) + v (P2 - P0) with 0\leq u,v and u+v \leq 1
        ...
        I = [6f0^2 + 4f0 (f1 - f0) + 4f0 (f2 - f0) + (f1-f0)(f2- f0) + (f1-f0)^2
       + (f2-f0)^2] / 12 = [f0^2 + f1^2 + f2^2 + f0 f1 + f0 f2 + f1 f2] / 12 The
       problem of this is that it requires solving a generalized eigenvalue
       problem. The dimensionality of the eigenvalues is [L^-1].
    */
    double val0 = h0 - B(i0);
    double val1 = h0 - B(i1);
    double val2 = h0 - B(i2);
    ...
    //    std::cerr << "i_elt=" << i_elt << " idx01=" << idx01 << " idx10=" <<
    //    idx10 << "\n"; std::cerr << "idx02=" << idx02 << " idx20=" << idx20 <<
    //    " idx12=" << idx12 << " idx21=" << idx21 << "\n";
  ...
  //    std::cerr << "i_adj=" << i_adj << " ListNbMatch[i_adj]=" <<
  //    ListNbMatch[i_adj] << "\n";
  ...
  std::vector<Ttrip> tripletList_A_SI, tripletList_A, tripletList_B,
      tripletList_B_SI;
  ...
  if (UseSI_Bmatrix) {
    ...
    std::cerr << "Using the B matrix computed from triangle integration and "
  ...
  for (int i_eig = 0; i_eig < nb_out; i_eig++) {
    ...
    std::cerr << "i_eig=" << i_eig << " VolSum=" << VolSum
              << " VolAbsSum=" << VolAbsSum << "\n";
    //
    double Norm_F_SI = 0;
    for (int ipt = 0; ipt < nb_point; ipt++) {
    ...
    for (int i_elt = 0; i_elt < nb_elt; i_elt++) {
      ...
      Norm_F_Int +=
          area * (f0 * f0 + f1 * f1 + f2 * f2 + f0 * f1 + f0 * f2 + f1 * f2);
    }
...
void WriteSeicheInfoAsNetcdfFile(std::vector<PeriodicSolution> const &ListSol,
  ...
  netCDF::NcVar eVarData_ocean =
      dataFile.addVar("ocean_time", "double", ListDimTime);
  eVarData_ocean.putAtt("units", "seconds since 1858-11-17 00:00:00");
  eVarData_ocean.putAtt("calendar", "gregorian");
  netCDF::NcVar eVarData_period =
      dataFile.addVar("periods", "double", ListDimTime);
  ...
  for (int i_sol = 0; i_sol < nb_sol; i_sol++) {
    ...
    std::cerr << "i_sol=" << i_sol << " : lambda=" << lambda
