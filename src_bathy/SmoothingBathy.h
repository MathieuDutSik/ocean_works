...
MyMatrix<double> GetRoughnessFactor(
    ...
    std::pair<GraphSparseImmutable, std::vector<std::pair<int, int>>> const
        &eGLP) {
  ...
  auto GetCoord = [&](int const &ePt) -> std::string {
    return "(" + std::to_string(eGLP.second[ePt].first) + ":" +
           std::to_string(eGLP.second[ePt].second) + ")";
  };
  std::cerr << "  AbsoluteMaxFrac=" << AbsoluteMaxFrac
            << " MatchingPair=" << GetCoord(MatchingPairIdx.first) << " / "
            << GetCoord(MatchingPairIdx.second) << "\n";
  std::cerr << "    MatchingPairDep=" << MatchingPairDep.first << " / "
            << MatchingPairDep.second << "\n";
...
MyVector<int> GetBadPoints(
    MyMatrix<double> const &DEP,
    std::pair<GraphSparseImmutable, std::vector<std::pair<int, int>>> const
        &eGLP,
...
MyMatrix<double> DoLinearProgrammingSmoothing(
    MyMatrix<double> const &DEP,
    std::pair<GraphSparseImmutable, std::vector<std::pair<int, int>>> const
        &eGLP,
    double const &rx0max, int const &NeighborLevel) {
  ...
  GraphSparseImmutable eGI =
      InducedSubgraph<GraphSparseImmutable, GraphSparseImmutable>(eGLP.first,
                                                                  eList);
  ...
  for (auto &eConn : ListConn) {
    ...
    int nbIneq = idx;
    std::cerr << "nbPair=" << nbPair << " dimProb=" << dimProb
              << " nbIneq=" << nbIneq << "\n";
    ...
    LpSolutionSimple<double> eSol = GLPK_LinearProgramming_Kernel_Sparse_PROC(
        Aspmat, ListAconst, Bspmat, ListBconst, ToBeMinimized, eGLPKoption);
    ...
    for (auto &ePair : ListPair) {
      ...
      if (delta < delta_crit) {
        std::cerr << "Error in our construction at ePt=" << ePt
                  << " fPt=" << fPt << "\n";
...
MyMatrix<double> DoMartinhoBatteenSmoothing(
    MyMatrix<double> const &DEP,
    std::pair<GraphSparseImmutable, std::vector<std::pair<int, int>>> const
        &eGLP,
    double const &rx0max) {
...
void DoFullSmoothing(FullNamelist const &eFull) {
  ...
  std::pair<GraphSparseImmutable, std::vector<std::pair<int, int>>> eGLP =
      GetGraphSparseVertexAdjacency(GrdArr);
  //
  ...
  if (TheMethod == "LinearProgramming") {
    DEPnew = DoLinearProgrammingSmoothing(GrdArr.GrdArrRho.DEP, eGLP, rx0max,
                                          NeighborLevel);
    DoOper = true;
  }
  ...
  if (!DoOper) {
    std::cerr
        << "Failed to find a matching function for bathymetry smoothing\n";
  ...
  for (int iPoint = 0; iPoint < nb_point; iPoint++) {
    std::pair<int, int> ePair = eGLP.second[iPoint];
    double deltaBathy = DEPnew(ePair.first, ePair.second) -
                        GrdArr.GrdArrRho.DEP(ePair.first, ePair.second);
